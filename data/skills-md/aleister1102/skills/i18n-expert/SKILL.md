---
name: i18n-expert
description: Use when setting up, auditing, or enforcing internationalization/localization in UI codebases (React/TS, i18next or similar, JSON locales), including configuring the framework, replacing hard-coded strings, ensuring en-US/zh-CN parity, and validating pluralization/messaging coverage.
---
...
## When to Use
- A React, Next.js, Vue, or similar UI project needs structured localization or audit coverage before release.
- You must enforce consistent en-US/zh-CN translations, pluralization, or metadata localization across components.
- You are mapping error codes, accessibility text, or SEO metadata into localized experiences for new markets.

## When NOT to Use
- The task is purely backend, infrastructure, or tooling work with no user-facing strings.
- Only one simple string change is required and a full localization workflow would slow progress.
- The project is intentionally single-language with no future localization plans.
