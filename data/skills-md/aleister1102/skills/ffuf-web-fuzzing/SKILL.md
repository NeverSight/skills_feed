...
description: Use when authenticated or unauthenticated web fuzzing with ffuf is required during a penetration test or bug bounty engagement.
---
...
## When to Use
- Mapping directories, files, or subdomains as part of an authenticated or unauthenticated pentest.
- Fuzzing parameters, headers, or POST bodies with complex wordlists or multi-template modes.
- Automating response filtering/auto-calibration to detect hidden content without manual noise filtering.

## When NOT to Use
- When you only need to port-scan or enumerate services; use `nmap` instead.
- When a quick, single URL check suffices (use curl or browsers for manual validation).
- When you prefer brute-forcing slow or resource-intensive APIs without rate limiting controls.
