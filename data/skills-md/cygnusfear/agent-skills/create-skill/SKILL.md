---
name: create-skill
description: This skill should be used when users want to create a new skill (or update an existing skill) that extends your capabilities with specialized knowledge, workflows, or tool integrations.
---
...
**Metadata Quality:** The `name` and `description` in YAML frontmatter determine when you will use the skill. Be specific about what the skill does and when to use it. Use third-person form (e.g., "This skill should be used when..." instead of "Use this skill when...").

**allowed-tools (optional):** Restricts which tools you can use within the skill. Leave empty to allow all tools (recommended unless restriction is needed).
...
3. **Bundled resources** - As needed by you (Unlimited*)

*Unlimited because scripts can be executed without reading into context window.

### Tool Restrictions with allowed-tools

The `allowed-tools` field restricts which tools you can use within the skill. Available tools include:
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another instance of you to use. Focus on including information that would be beneficial and non-obvious to you. Consider what procedural knowledge, domain-specific details, or reusable assets would help another instance execute these tasks more effectively.
...
3. In practice, how should you use the skill? All reusable skill contents developed above should be referenced so that you know how to use them.
...
2. **Notice struggles** - Identify where you struggle or is inefficient
...
**Skills** are model-invoked (you decide when to use them based on context).
